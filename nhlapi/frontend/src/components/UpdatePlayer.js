...
const UpdatePlayer = () => {
    ...
    const handleSubmit = async(e) => {
            ...
            !height.current.value.length > 0 && !weight.current.value.length > 0 && !jersey.current.value.length > 0)
        {
            ...
            await fetch(conf.url + conf.endpoints.players + `/${id}?key=${JSON.parse(localStorage.getItem("user")).key}`, {
                method: "PUT",
                mode: "cors",
                body: JSON.stringify(
                    {
    ...
    return (
        <div className='flex h-fit justify-center'>
            <div className="flex flex-col gap-5 bg-slate-700 w-[30%] h-fit text-center p-5 rounded-xl shadow-lg">
                <h1 className="text-5xl text-white">Update Player</h1>
                <form onSubmit={handleSubmit} className='flex flex-col gap-10'>
                    <input type="text" placeholder={player.first_name} className="w-[75%] p-5 rounded-xl text-xl mx-auto" ref={firstName}/>
                    <input type="text" placeholder={player.last_name} className='w-[75%] p-5 rounded-xl text-xl mx-auto' ref={lastName}/>
                    <input type="text" disabled value={player.birth_date} className='w-[75%] p-5 rounded-xl text-xl mx-auto' ref={birthdate}/>
                    <input type="text" disabled value={player.birth_city} className='w-[75%] p-5 rounded-xl text-xl mx-auto' ref={birthCity}/>
                    <input type="text" disabled value={player.birth_state} className='w-[75%] p-5 rounded-xl text-xl mx-auto' ref={birthState}/>
                    <input type="text" placeholder={player.height} className='w-[75%] p-5 rounded-xl text-xl mx-auto' ref={height}/>
                    <input type="text" placeholder={player.weight} className='w-[75%] p-5 rounded-xl text-xl mx-auto' ref={weight}/>
                    <input type="text" placeholder={player.jersey} className='w-[75%] p-5 rounded-xl text-xl mx-auto' ref={jersey}/>
                    ...
                    <input type="submit" value="Update" className='rounded-xl shadow-lg text-2xl h-16 w-fit p-2 mx-auto bg-slate-200'/>
                </form>
                {
                  error?
